...
nngn.entities:set_max(6)
nngn.graphics:resize_textures(7)
nngn.textures:set_max(7)
...
local HIST_SIZE = 4 * 256
local HIST_BYTES = 4 * HIST_SIZE

local LIMITS = nngn.compute:get_limits()
local COMPUTE_UNITS = LIMITS[Compute.COMPUTE_UNITS + 1]
local GLOBAL_SIZE = common.round_down_pow2(COMPUTE_UNITS)
local LOCAL_SIZE = math.min(IMG_SIZE // 2, LIMITS[Compute.WORK_GROUP_SIZE + 1])
local LOCAL_SIZE_SQRT = math.sqrt(LOCAL_SIZE)
...
local img_raw = nngn.textures.read("img/petrov.png")
