...
local function exec(out_type, sizeof_out, out_size, ...)
    local out = assert(
        nngn.compute:create_buffer(
            Compute.WRITE_ONLY, out_type, out_size))
    local exec_args = {...}
    local n_args = #exec_args
    local args = {table.unpack(exec_args[n_args])}
    table.insert(args, Compute.BUFFER)
    table.insert(args, out)
    exec_args[n_args] = args
    table.insert(exec_args, {})
    n_args = n_args + 1
    common.print_prof(common.avg_prof(5, function(e)
        exec_args[n_args + 1] = e
        assert(nngn.compute:execute(table.unpack(exec_args)))
    end))
...
local function test_mul_mat_cpu(n)
    ...
    print(string.format(
        "mul_mat_cpu(%d): %.3fs",
        n, nngn.timing.time_ms(
            function() nngn.math.mat_mul(n, m0, m0, m2) end)))
end
...
local function test_mul_mat(n)
    ...
    for _, t in ipairs(ts) do
        ...
        io.write(f, ": ", n, "\n")
        local out = exec(
            Compute.FLOATV, Compute.SIZEOF_FLOAT, n2, prog, f, Compute.BLOCKING,
            g, l, a)
        if not common.vec_eq(out, cmp) then error("check failed") end
    end
    assert(nngn.compute:release_buffer(b1))
