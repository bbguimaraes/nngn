...
nngn.schedule:next(Schedule.HEARTBEAT, function()
    ...
    local t = now % 10000 / 10000
    nngn.renderers:set_sphere_interp_normals(t < 0.5)
    t = t * 2 % 1
    if t < 0.5 then
        t = t * 2
    else
    ...
    local sd = t ^ 2 * (DIV - 1) // 1 + 2
    if sd ~= sphere_div then
        sphere_div = sd
        nngn.renderers:set_sphere_divisions(sd, sd)
    end
