...
bool WAV::check(void) const {
    ...
    const auto check_seg = [this](
        ...
        if(std::equal(begin(sc), end(sc), begin(cmp), end(cmp)))
            return true;
        auto &l = Log::l();
        l << "invalid " << name << " segment ID: ";
        std::copy(begin(sc), end(sc), std::ostream_iterator<char>{l});
        l << " != ";
        std::copy(begin(cmp), end(cmp), std::ostream_iterator<char>{l});
        l << '\n';
        return false;
    };
