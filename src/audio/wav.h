...
public:
    ...
    WAV(void) = default;
    constexpr explicit WAV(std::span<std::byte> buffer);
...
inline constexpr WAV::WAV(std::span<std::byte> buffer) : b{buffer} {
    assert(WAV::HEADER_SIZE <= this->b.size());
};
...
inline constexpr std::span<std::byte> WAV::data(void) const {
    assert(this->size() == this->b.size() - WAV::HEADER_SIZE);
