...
struct Pseudocomp : public nngn::Compute {
    ...
    void get_limits(std::uint64_t *p) const override;
    ...
    std::size_t n_events(std::size_t, const Type*) const override
        { return true; }
    bool prof_info(ProfInfo, std::size_t, const Event*, std::uint64_t*)
        const override
        { return true; }
    bool wait(std::size_t, const Event*) const override { return true; }
    bool release_events(std::size_t, const Event*) const override
        { return true; }
    bool execute(
            Program, const std::string&, ExecFlag,
            std::uint32_t, const std::size_t*, const std::size_t*,
            std::size_t, const Type*, const std::size_t*,
            const std::byte *const*, Events) const override
        { return true; }
};

void Pseudocomp::get_limits(std::uint64_t *p) const {
