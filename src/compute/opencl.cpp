...
public:
    ...
    bool prof_info(
        ProfInfo info, std::size_t n, const Event *const *events,
        u64 *out) const final;
...
cl_event *to_cl_events(nngn::Compute::Event *const *e) {
    return const_cast<cl_event*>(reinterpret_cast<const cl_event*>(e));
}

cl_event *to_cl_events(const nngn::Compute::Event *const *e) {
    return to_cl_events(const_cast<nngn::Compute::Event *const*>(e));
}

nngn::Compute::Event *const *from_cl_event(cl_event *e) {
    return reinterpret_cast<nngn::Compute::Event *const*>(e);
}
...
void release(cl_program x) { (void)LOG_RESULT(clReleaseProgram, x); }
void release(cl_kernel x) { (void)LOG_RESULT(clReleaseKernel, x); }
void release(cl_sampler x) { (void)LOG_RESULT(clReleaseSampler, x); }
void release(cl_mem x) { (void)LOG_RESULT(clReleaseMemObject, x); }
void release(cl_command_queue x) { (void)LOG_RESULT(clReleaseCommandQueue, x); }
void release(cl_context x) { (void)LOG_RESULT(clReleaseContext, x); }
...
OpenCLBackend::~OpenCLBackend() {
    ...
    static constexpr auto r = [](auto x) { if(x) release(x); };
    constexpr auto f = [](const auto &v) { for(auto x : v) r(x); };
    f(this->programs);
    f(this->kernels);
    f(this->samplers);
    f(this->images);
...
std::string OpenCLBackend::platform_name() const {
    ...
    if(LOG_RESULT(clGetPlatformInfo, this->platform, i, 0, nullptr, &n)) {
        ret.resize(n);
        (void)LOG_RESULT(clGetPlatformInfo,
...
std::string OpenCLBackend::device_name() const {
    ...
    if(LOG_RESULT(clGetDeviceInfo, this->device, i, 0, nullptr, &n)) {
        ret.resize(n);
        (void)LOG_RESULT(clGetDeviceInfo,
            this->device, i, n, ret.data(), nullptr);
    }
    return ret;
}
...
auto OpenCLBackend::create_kernel(
    ...
    if(!this->set_kernel_args(
            k, len, types, sizes, data, to_cl_events(events.events))) {
...
bool OpenCLBackend::prof_info(
    ...
    const auto f = [info, &out](auto i, auto e, auto v) {
        ...
        CHECK_RESULT(clGetEventInfo,
            *to_cl_events(&e), CL_EVENT_COMMAND_EXECUTION_STATUS,
        ...
        CHECK_RESULT(clGetEventProfilingInfo,
            *to_cl_events(&e), static_cast<cl_profiling_info>(v),
...
bool OpenCLBackend::wait(std::size_t n, const Event *const *v) const {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
    return LOG_RESULT(clWaitForEvents,
        static_cast<cl_uint>(n), to_cl_events(v));
}

bool OpenCLBackend::release_events(std::size_t n, const Event *const *v) const {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
    return std::all_of(v, v + n, [](auto &x)
        { return LOG_RESULT(clReleaseEvent, *to_cl_events(&x)); });
}
...
bool OpenCLBackend::execute_(
    ...
    if(const auto n = static_cast<cl_uint>(n_events - 1)) {
        CHECK_RESULT(clEnqueueNDRangeKernel,
            ...
            n, to_cl_events(events.events), event);
    } else
        CHECK_RESULT(clEnqueueNDRangeKernel,
            this->queue, kernel, n_dim, nullptr, global_size, local_size,
            0, nullptr, event);
...
bool OpenCLBackend::execute(
    ...
    return set_kernel_args(
            *kernel, len, types, sizes, data, to_cl_events(events.events),
            &*tmp_buffers)
        && this->execute_(
