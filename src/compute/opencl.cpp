...
template<> std::unique_ptr<Compute> compute_create_backend<
        Compute::Backend::OPENCL_BACKEND>(const void*) {
...
struct Platform {
    cl_platform_id id = {};
    std::vector<cl_device_id> gpus = {}, cpus = {};
};
...
class OpenCLBackend final : public nngn::Compute {
    ...
    struct limits {
        uint64_t compute_units = 0, work_group_size = 0, local_memory = 0;
    } limits = {};
public:
    NNGN_NO_COPY(OpenCLBackend)
    OpenCLBackend(OpenCLParameters p);
    ~OpenCLBackend() final;
    bool init() final;
    size_t n_platforms() const final;
    size_t n_devices() const final;
    void get_limits(uint64_t *p) const final
        { std::memcpy(p, &this->limits, sizeof(this->limits)); }
    std::string platform_name() const final;
    std::string device_name() const final;
    Buffer create_buffer(MemFlag flags, size_t n, const std::byte *p) final;
    bool read_buffer(
        Buffer b, size_t off, size_t n, std::byte *p,
        Events events) const final;
    bool fill_buffer(
        Buffer b, size_t off, size_t n, std::byte v,
        Events events) const final;
    bool fill_buffer(
        Buffer b, size_t off, size_t n,
        size_t pattern_size, const std::byte *p, Events events) const final;
    bool write_buffer(
        Buffer b, size_t off, size_t n, const std::byte *p,
        Events events) const final;
    ...
    void *map_buffer(
        Buffer b, MemFlag flags, size_t off, size_t n,
        Events events) const final;
    bool unmap_buffer(Buffer b, void *p, Events events) const final;
    bool release_buffer(Buffer b) final;
    Image create_image(
        Type type, size_t w, size_t h, MemFlag flags,
        const std::byte *p) final;
    bool read_image(
        Image i, size_t w, size_t h, std::byte *p,
        Events events) const final;
    bool fill_image(
    ...
    size_t n_events(size_t n, const Type *types) const final;
    bool prof_info(
        ProfInfo info, size_t n, const Event *events,
        uint64_t *out) const final;
    bool wait(size_t n, const Event *e) const final;
    bool release_events(size_t n, const Event *v) const final;
    bool execute(
        Program program, const std::string &func, ExecFlag flags,
        uint32_t n_dim, const size_t *global_size, const size_t *local_size,
        size_t len, const Type *types,
        const size_t *sizes, const std::byte *const *data,
        Events events) const final;
};
...
template<typename T>
uint32_t insert_at_first_free(std::vector<T> *v, T t) {
    const auto b = v->begin(), e = v->end();
    if(auto const it = std::find(b + 1, e, T{}); it != e) {
        *it = t;
        return static_cast<uint32_t>(std::distance(b, it));
    }
    v->emplace_back(t);
...
bool OpenCLBackend::init() {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend)
    const auto get_platforms = [](auto *v) {
    ...
    const auto get_devices = [](auto p, auto name, auto type, auto *v) {
        NNGN_LOG_CONTEXT(name);
        cl_uint n = 0;
        const auto t = static_cast<cl_device_type>(type);
        const auto err = clGetDeviceIDs(p, t, 0, nullptr, &n);
        switch(err) {
        ...
        v->resize(n);
        CHECK_RESULT(clGetDeviceIDs, p, t, n, v->data(), nullptr);
        return true;
    };
    const auto select_device = [](const auto &ps, auto mp, auto *p, auto *d) {
        for(const auto &x : ps)
            if(const auto &v = x.*mp; !v.empty())
                return *p = x.id, *d = v[0], true;
        return false;
    };
    const auto get_device_info = [this](auto *l) {
        using T = std::tuple<uint32_t, uint64_t*>;
        for(auto [i, p] : {
            ...
            CHECK_RESULT(clGetDeviceInfo,
                this->device, i, sizeof(*p), p, nullptr);
        return true;
    };
    const auto create_context = [this](auto *p) {
        const auto cb = [](const char *errinfo, const void*, size_t, void*) {
    ...
    if(!get_platforms(&this->platforms))
        return false;
    for(auto &x : this->platforms) {
        if(!get_devices(x.id, "gpus", CL_DEVICE_TYPE_GPU, &x.gpus))
            return false;
        if(!get_devices(x.id, "cpus", CL_DEVICE_TYPE_CPU, &x.cpus))
            return false;
    }
    switch(this->preferred_device) {
    case DeviceType::GPU:
        select_device(
            this->platforms, &Platform::gpus, &this->platform, &this->device)
        || select_device(
            this->platforms, &Platform::cpus, &this->platform, &this->device);
        break;
    case DeviceType::CPU:
    default:
        select_device(
    ...
    if(!get_device_info(&this->limits))
        return false;
...
size_t OpenCLBackend::n_platforms() const { return this->platforms.size(); }

size_t OpenCLBackend::n_devices() const {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
    return std::accumulate(
        begin(this->platforms), end(this->platforms), std::size_t{},
        [](auto a, const auto &x)
            { return a + x.gpus.size() + x.cpus.size(); });
}
...
auto OpenCLBackend::create_buffer(
        MemFlag mem_flags, size_t n, const std::byte *p)
        -> Buffer {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
    cl_int err = CL_SUCCESS;
...
bool OpenCLBackend::read_buffer(
        Buffer b, size_t off, size_t n, std::byte *p, Events events) const {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
...
bool OpenCLBackend::fill_buffer(
        Buffer b, size_t off, size_t n, std::byte v, Events events) const {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
...
bool OpenCLBackend::fill_buffer(
        Buffer b, size_t off, size_t n,
        size_t pattern_size, const std::byte *p, Events events) const {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
...
void *OpenCLBackend::map_buffer(
        Buffer b, MemFlag mem_flags, size_t off, size_t n,
        Events events) const {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
...
auto OpenCLBackend::create_image(
        Type type, size_t w, size_t h, MemFlag mem_flags, const std::byte *p)
        -> Image {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
    cl_image_format fmt = {};
...
bool OpenCLBackend::read_image(
        ...
        Events events) const {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
    constexpr std::array<size_t, 3> origin = {};
    const std::array<size_t, 3> region = {w, h, 1};
...
auto OpenCLBackend::create_sampler() -> Sampler {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
    cl_int err = CL_SUCCESS;
    const auto s = clCreateSampler(
        this->context, false, CL_ADDRESS_CLAMP_TO_EDGE, CL_FILTER_LINEAR, &err);
...
auto OpenCLBackend::create_program(
        std::string_view src, const char *opts) -> Program {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
    auto src_p = src.data();
...
size_t OpenCLBackend::n_events(size_t n, const Type *v) const {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
    return static_cast<size_t>(std::count_if(v, v + n, is_vector_type));
}

bool OpenCLBackend::prof_info(
        ProfInfo info, size_t n, const Event *events, uint64_t *out) const {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
...
bool OpenCLBackend::wait(size_t n, const Event *v) const {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
    return LOG_RESULT(clWaitForEvents,
        static_cast<cl_uint>(n), to_cl_event(v));
}

bool OpenCLBackend::release_events(size_t n, const Event *v) const {
    NNGN_LOG_CONTEXT_CF(OpenCLBackend);
...
bool OpenCLBackend::execute(
    ...
    auto tmp_buffers = nngn::scoped(
        std::vector<cl_mem>{},
        [](const auto &v) { for(auto x : v) if(x) clReleaseMemObject(x); });
    for(size_t i = 0; i < len; ++i) {
        const auto set = []<typename T>(
                const auto *name,
                const auto &v, const auto *p, auto k, auto ui, T t) {
            NNGN_LOG_CONTEXT(name);
            t.id = *static_cast<const uint32_t*>(static_cast<const void*>(p));
            const auto *x = get_obj(v, t);
            return LOG_RESULT(clSetKernelArg, k, ui, sizeof(*x), x);
        };
        ...
        case Type::LOCAL: {
            ...
            CHECK_RESULT(clSetKernelArg, *kernel, ui, s, nullptr);
            break;
        }
        case Type::BYTE:
            CHECK_RESULT(clSetKernelArg, *kernel, ui, sizeof(cl_uchar), d);
            break;
        case Type::INT:
            CHECK_RESULT(clSetKernelArg, *kernel, ui, sizeof(cl_int), d);
            break;
        case Type::UINT:
            CHECK_RESULT(clSetKernelArg, *kernel, ui, sizeof(cl_uint), d);
            break;
        case Type::FLOAT:
            CHECK_RESULT(clSetKernelArg, *kernel, ui, sizeof(cl_float), d);
            break;
        case Type::BYTEV:
        case Type::INTV:
        case Type::UINTV:
        case Type::FLOATV: {
            ...
            CHECK_RESULT(clSetKernelArg,
                *kernel, static_cast<cl_uint>(i), sizeof(cl_mem), &buffer);
            break;
        }
        case nngn::Compute::Type::DATA:
            CHECK_RESULT(clSetKernelArg, *kernel, ui, sizes[i], d);
            break;
        case nngn::Compute::Type::BUFFER:
            if(!set("buffer", this->buffers, d, *kernel, ui, Buffer{}))
                return false;
            break;
        case nngn::Compute::Type::IMAGE:
            if(!set("image", this->images, d, *kernel, ui, Image{}))
                return false;
            break;
        case nngn::Compute::Type::SAMPLER:
            if(!set("sampler", this->samplers, d, *kernel, ui, Sampler{}))
                return false;
            break;
...
template<> std::unique_ptr<Compute> compute_create_backend
        <Compute::Backend::OPENCL_BACKEND>(const void *params) {
