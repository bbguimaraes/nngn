...
bool Map::set_max(std::size_t n) {
    ...
    if(!this->m_vbo) {
        const u32 new_vbo = this->graphics->create_buffer({
            .name = "map_vbo",
        ...
        if(!new_vbo)
            return false;
        const u32 new_ebo = this->graphics->create_buffer({
            .name = "map_ebo",
        ...
        this->graphics->set_buffer_capacity(this->m_vbo, vsize)
        && this->graphics->set_buffer_capacity(this->m_ebo, esize)
    ))
        return false;
    if(!(
        this->graphics->set_buffer_size(this->m_vbo, 0)
        && this->graphics->set_buffer_size(this->m_ebo, 0)
    ))
        return false;
    this->max = n;
    return true;
...
std::vector<uvec2> Map::load_tiles(
    std::size_t width, std::size_t height, nngn::lua::table_view t)
{
    const auto n = width * height;
    const auto ni = nngn::narrow<lua_Integer>(2 * n);
    std::vector<uvec2> ret = {};
    ret.reserve(n);
    for(lua_Integer i = 1; i <= ni; i += 2)
