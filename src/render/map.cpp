...
bool Map::gen() const {
    ...
    constexpr auto egen = [](void*, void *p, std::uint64_t i, std::uint64_t n) {
        nngn::Renderers::gen_quad_idxs(i, n, static_cast<std::uint32_t*>(p));
    };
    ...
    if(!ok)
        return false;
    this->graphics->set_buffer_size(this->m_vbo, n * 4 * sizeof(Vertex));
    this->graphics->set_buffer_size(this->m_ebo, n * 6 * sizeof(u32));
    return true;
}
