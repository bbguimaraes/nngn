...
void check_sphere_plane(
    const std::vector<SphereCollider> &sphere,
    const std::vector<PlaneCollider> &plane,
    Output *output);
...
template<typename T, typename U> bool old_add_collision(
...
bool NativeBackend::check(
    ...
    check_sphere(data, input.sphere, output);
...
void check_sphere(
        const data_t &data,
        const std::vector<SphereCollider> &sphere, Output *output) {
...
void check_plane(const std::vector<PlaneCollider>&, Output *output) {
    NNGN_STATS_CONTEXT(Colliders, &output->stats.plane);
    // TODO
//    nngn::vec3 v = {};
//    for(auto i0 = plane.cbegin(), e = plane.cend(); i0 != e; ++i0) {
//        const auto &c0 = *i0;
//        for(auto i1 = i0 + 1; i1 != e; ++i1) {
//            const auto &c1 = *i1;
//            {
...
void check_sphere_plane(
    ...
    if(plane.empty())
        return;
    nngn::vec3 v = {};
    for(const auto &c0 : sphere)
        for(const auto &c1 : plane) {
...
constexpr nngn::vec2 rotate(const nngn::vec2 &p, float cos, float sin)
    { return nngn::vec2(p.x * cos - p.y * sin, p.x * sin + p.y * cos); }
