...
static auto font(const Fonts *f) { assert(f); return f->fonts() + f->n() - 1; }

std::size_t Textbox::text_length() const {
    constexpr auto f = [](const auto &t) {
        const auto b = t.str.begin();
        return static_cast<uint32_t>(std::count_if(
            b, b + static_cast<ptrdiff_t>(t.cur),
...
void Textbox::set_cur(size_t cur)
...
bool Textbox::update(const nngn::Timing &t) {
    ...
    const auto n = static_cast<size_t>(this->timer / this->speed);
    size_t c = 0;
...
void Textbox::update_size(const uvec2 &screen) {
    ...
    const auto fscreen = static_cast<float>(screen.x);
    const auto pad = static_cast<float>(f->size) / 2.0f;
    const auto str_sz = vec2(WIDTH - 2 * pad, this->str.size.y + 2 * pad);
    const auto center = vec2(fscreen / 2.0f, pad + str_sz.y / 2.0f);
    this->str_bl = center - str_sz / 2.0f;
    this->str_tr = str_bl + str_sz;
    const auto title_sz = this->title.size + pad * vec2(2);
    this->title_bl = this->str_bl + vec2(pad, str_sz.y - pad);
