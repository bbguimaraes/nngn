...
void Text::update_cur(const Font &f, size_t c) {
...
size_t Text::count_lines(std::string_view s, size_t cur) {
    assert(cur <= s.size());
    const auto *const b = s.cbegin();
    return 1 + static_cast<size_t>(
        std::count(b, b + static_cast<ptrdiff_t>(cur), '\n'));
}

float Text::max_width(
        const Font::Characters &font,
        std::string_view str, size_t cur) {
    assert(!cur || font.size() >= 128);
    const auto *const end = str.cbegin() + static_cast<ptrdiff_t>(cur);
    assert(end <= str.cend());
    const auto acc = [&font](float a, char c)
        { return a + font[static_cast<size_t>(c)].advance; };
    float ret = 0.0f;
