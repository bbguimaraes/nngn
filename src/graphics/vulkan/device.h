...
public:
    ...
    ~Device();
    VkPhysicalDevice physical_dev() const { return this->ph; }
    VkDevice id() const { return this->h; }
    /** Family of the queue returned by \ref graphics_queue. */
    std::uint32_t graphics_family() const { return this->families.graphics; }
    /** Family of the queue returned by \ref present_queue. */
    std::uint32_t present_family() const { return this->families.present; }
    /** Queue used for graphics operations. */
    VkQueue graphics_queue() const { return this->queues.graphics; }
    /** Queue used for presentation, may be the same as \ref graphics_queue. */
    VkQueue present_queue() const { return this->queues.present; }
    /**
