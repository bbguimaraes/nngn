...
std::vector<Graphics::Extension> vk_parse_extensions(
    ...
    for(const auto &x : s) {
        auto &e = ret.emplace_back(
            Graphics::Extension{.name = {}, .version = x.specVersion});
...
std::vector<Graphics::PresentMode> vk_parse_present_modes(
    ...
    std::vector<Graphics::PresentMode> ret;
    ret.reserve(s.size());
    for(auto x : s) {
        Graphics::PresentMode m = {};
        switch(x) {
        case VK_PRESENT_MODE_IMMEDIATE_KHR:
            m = Graphics::PresentMode::IMMEDIATE; break;
        case VK_PRESENT_MODE_MAILBOX_KHR:
            m = Graphics::PresentMode::MAILBOX; break;
        case VK_PRESENT_MODE_FIFO_KHR:
            m = Graphics::PresentMode::FIFO; break;
        case VK_PRESENT_MODE_FIFO_RELAXED_KHR:
            m = Graphics::PresentMode::FIFO_RELAXED; break;
