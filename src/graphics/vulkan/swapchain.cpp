...
bool SwapChain::create_img_views(const Instance &inst) {
    NNGN_LOG_CONTEXT_CF(SwapChain);
    const auto imgs =
        vk_enumerate<VkImage, vkGetSwapchainImagesKHR>(this->dev, this->h);
...
auto SwapChain::acquire_img() -> std::pair<PresentContext, VkResult> {
    ...
    return {{
        ...
        .fence = this->fences.render[i]}, VK_SUCCESS};
}

void SurfaceInfo::init(VkSurfaceKHR s, VkPhysicalDevice d) {
    this->formats = vk_enumerate<
        VkSurfaceFormatKHR,
        vkGetPhysicalDeviceSurfaceFormatsKHR>(d, s);
    this->present_modes = vk_parse_present_modes(
        vk_enumerate<
            VkPresentModeKHR,
            vkGetPhysicalDeviceSurfacePresentModesKHR>(d, s));
