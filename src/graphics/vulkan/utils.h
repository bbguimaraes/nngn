 ...
 */
template<typename T, function_pointer auto f>
std::vector<T> vk_enumerate(const auto &...args);

/** Populates \ref Extension objects from their Vulkan equivalents. */
std::vector<Graphics::Extension> vk_parse_extensions(
    std::span<const VkExtensionProperties> s);

/** Populates \ref PresentMode objects from their Vulkan equivalents. */
