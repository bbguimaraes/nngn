...
public:
    NNGN_NO_COPY(ShadowCube)
    ShadowCube() : ShadowMap(nngn::Graphics::SHADOW_CUBE_INITIAL_SIZE) {}
    ~ShadowCube() = default;
...
public:
    ...
    bool init_device() final { return this->init_device(SIZE_MAX); }
    ...
    void set_buffer_size(u32, u64 size) final;
...
constexpr VkDeviceSize CameraDescriptorSets::size() {
    constexpr std::size_t
        normal = 1, hud = 1,
        n_lights = nngn::Lighting::MAX_LIGHTS,
...
bool VulkanBackend::create_cmd_buffer(std::size_t img_idx) {
    ...
    const auto record_render_pass = [
        ...
        render(b,
            *this->camera.flags & nngn::Camera::Flag::PERSPECTIVE
            ...
            viewport, scissors,
            *this->camera.flags & nngn::Camera::Flag::PERSPECTIVE
                ? this->render_list.map_persp : this->render_list.map_ortho);
    ...
    if(lights.flags & nngn::LightsUBO::SHADOWS_ENABLED) {
        ...
        for(std::size_t l = 0, nl = 6 * lights.n_point; l < nl; ++l)
...
bool VulkanBackend::set_shadow_map_size(std::uint32_t s) {
    ...
    return this->shadow_map.init(
            ...
            this->n_frames, nngn::Lighting::MAX_LIGHTS,
            {s, s, 1})
        && this->instance.set_obj_name(
...
bool VulkanBackend::set_shadow_cube_size(std::uint32_t s) {
    NNGN_LOG_CONTEXT_CF(VulkanBackend);
    constexpr auto n = 6 * nngn::Lighting::MAX_LIGHTS;
    this->shadow_cube.destroy(this->dev.id(), &this->dev_mem);
...
void VulkanBackend::set_buffer_size(u32 b, u64 size) {
...
bool VulkanBackend::set_n_frames(std::size_t n) {
    ...
    this->ubo.memcpy(
        ...
        nngn::as_byte_span(&nngn::Lighting::NO_LIGHTS_UBO));
...
bool VulkanBackend::load_font(
    ...
    for(auto i = c; i < e; ++i) {
        ...
        if(!this->stg_buffer.write(this->cur_frame, s.y, 4 * s.x, write))
            return false;
    }
    this->font_tex.transition_layout(
...
bool VulkanBackend::render() {
    ...
    const auto update_camera = [this] {
        ...
        this->ubo.memcpy(
            this->dev.id(),
            this->camera_descriptor_sets.offset_hud(this->cur_frame),
            nngn::as_byte_span(nngn::rptr(nngn::CameraUBO{
                .proj = CLIP_PROJ * *this->camera.hud_proj})));
    };
    const auto update_lighting = [this] {
        ...
        constexpr auto max_lights = nngn::Lighting::MAX_LIGHTS;
