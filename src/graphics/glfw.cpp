...
bool GLFWBackend::create_window(void) {
...
uvec2 GLFWBackend::window_size(void) const {
    ivec2 ret;
...
void GLFWBackend::get_keys(size_t n, int32_t *keys) const {
    std::transform(keys, keys + n, keys, [w_ = this->w](auto x) {
        return static_cast<char>(glfwGetKey(w_, x));
    });
}

ivec2 GLFWBackend::mouse_pos(void) const {
    dvec2 ret;
