...
class TerminalBackend final : public nngn::Pseudograph {
    ...
    bool render() final;
public:
    NNGN_NO_COPY(TerminalBackend)
    explicit TerminalBackend(int fd) : term(fd) {}
    ~TerminalBackend() final = default;
};
...
void TerminalBackend::set_buffer_size(u32 b, u64 s) {
