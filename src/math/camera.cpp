...
const float Camera::TAN_FOVY = tanf(FOVY);
const float Camera::TAN_HALF_FOVY = tanf(FOVY / 2);
...
void Camera::set_screen(const uvec2 &s) {
    ...
    this->flags |= Flag::SCREEN_UPDATED;
}
...
void Camera::look_at(const vec3 &pos, const vec3 &eye, const vec3 &up) {
    ...
    const auto dir = Math::normalize(pos - eye);
    const auto ay = Math::angle(
        camera_dir, Math::normalize(vec3(dir.x, 0, dir.z)), up);
    const auto ax = Math::angle(
        camera_dir, vec3(0, Math::rotate_y(dir, -ay).yz()), camera_lat);
    this->p = eye;
    this->r = {ax, ay, 0};
    this->flags |= Flag::UPDATED;
}
