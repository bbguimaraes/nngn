...
concept tracker_with_realloc =
    requires(T t, T::pointer p, std::size_t n) {
...
public:
    ...
    using allocator =
        typename std::allocator_traits<A>::rebind_alloc<value_type>;
private:
    template<typename I>
...
public:
    ...
    struct rebind {
        using other =
            tracking_allocator<typename T::rebind<U>::other, allocator>;
    };
    ...
    tracking_allocator(const tracking_allocator<D1, A1> &rhs)
...
template<typename T, typename A>
auto tracking_allocator<T, A>::reallocate(pointer p, std::size_t n)
    noexcept -> pointer
    requires detail::has_realloc<allocator>
{
