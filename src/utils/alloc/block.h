...
public:
    ...
    void realloc(std::size_t s) requires both_trivial;
...
template<typename H, typename T>
inline auto alloc_block<H, T>::alloc(void)
    -> alloc_block requires both_trivial
{
    return alloc_block::alloc(sizeof(alloc_block::value_type));
}

template<typename H, typename T>
inline auto alloc_block<H, T>::alloc(std::size_t s)
    -> alloc_block requires both_trivial
{
    alloc_block ret;
    ret.realloc(s);
...
template<typename H, typename T>
inline void alloc_block<H, T>::realloc(std::size_t s) requires both_trivial {
