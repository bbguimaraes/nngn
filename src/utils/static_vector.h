...
public:
     ...
     */
    std::size_t size() const { return vector_type::size() - this->n_free(); }
...
template<typename T>
bool static_vector<T>::full() const
    { return !this->free_head && vector_type::size() == this->capacity(); }

template<typename T>
std::size_t static_vector<T>::n_free() const {
    std::size_t ret = {};
