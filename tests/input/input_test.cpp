...
void InputTest::get_keys() {
    ...
    input.get_keys({&v, 1});
    QVERIFY(!v);
    struct final : Input::Source {
        void get_keys(std::span<i32> s) const final { s[0] = 1; }
    } s;
...
void InputTest::get_keys_override() {
    ...
    i32 v = key;
    Input input;
    input.override_keys(true, {&v, 1});
    struct final : Input::Source {
        void get_keys(std::span<i32>) const final {}
    } s;
    input.add_source(std::make_unique<decltype(s)>(s));
    input.get_keys({&v, 1});
    QCOMPARE(v, 1);
...
void InputTest::override_keys() {
    ...
    constexpr auto keys = std::to_array<i32>({'K', 'E', 'Y', 'S'});
    ...
    QCOMPARE(str(v), "0000");
    v = keys;
    QVERIFY(input.override_keys(true, {&v[0], 1}));
    QVERIFY(input.override_keys(true, {&v[2], 1}));
    v = keys;
    ...
    QCOMPARE(str(v), "1010");
    v = keys;
    QVERIFY(input.override_keys(false, {&v[0], 1}));
    QVERIFY(input.override_keys(false, {&v[2], 1}));
    input.get_keys(v);
    QCOMPARE(str(v), "0000");
}
