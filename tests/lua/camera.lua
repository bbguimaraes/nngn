...
local function test_reset_ortho()
    ...
    camera.reset(1)
    test_reset_common(c)
    common.assert_eq({c:pos()}, {0, 0, -1}, common.deep_cmp)
...
local function test_reset_persp()
    ...
    test_reset_common(c)
    local eye = {c:eye()}
    common.assert_eq(eye[1], 0)
    common.assert_eq(eye[2], math.sin(a) * -fov_z / zoom, cmp)
    common.assert_eq(eye[3], math.cos(a) *  fov_z / zoom, cmp)
    local rot = {c:rot()}
...
local function test_reset_follow()
    ...
    nngn.entities:set_pos(e, 1, 2, 3)
    camera.set_follow(e)
    camera.reset()
    common.assert_eq({c:eye()}, {1, 2, 0}, common.deep_cmp)
    camera.set_follow(nil)
    camera.reset()
    common.assert_eq({c:eye()}, {0, 0, 0}, common.deep_cmp)
